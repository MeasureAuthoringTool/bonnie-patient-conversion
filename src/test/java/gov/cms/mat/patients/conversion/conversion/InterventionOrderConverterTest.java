...
@SpringBootTest
@ActiveProfiles("test")
class InterventionOrderConverterTest extends BaseConversionTest implements FhirConversionTest, ServiceRequestCommonTest {
    private final ServiceRequest.ServiceRequestIntent FHIR_INTENT = ServiceRequest.ServiceRequestIntent.ORDER;

    @Autowired
    private InterventionOrderConverter interventionOrderConverter;

    @Test
    void getQdmType() {
        assertEquals(InterventionOrderConverter.QDM_TYPE, interventionOrderConverter.getQdmType());
    }
    ...
    void convertToFhirWithoutNegation() {
        ...
        QdmToFhirConversionResult<ServiceRequest> result = interventionOrderConverter.convertToFhir(fhirPatient, qdmDataElement);
